...
#define STATEMACHINE_WAIT_FOR_RFID_DETECTED_TIMEOUT		5000
#define STATEMACHINE_WAIT_FOR_RFID_UPDATE_TIMEOUT		5000
...
static void STATEMACHINE_waitForRFIDDetected(void){
	...
	if(STATEMACHINE_timeoutFlag){
		STATEMACHINE_dispenseResultCallback(PROTOCOL_RESULT_CMD_DISPENSE_CARD_TIMEOUT);
...
static void STATEMACHINE_updateRFID(void){
	...
	if(!RFID_set(usingRfidId, &STATEMACHINE_usingRfid)){
		STATEMACHINE_dispenseResultCallback(PROTOCOL_RESULT_CMD_DISPENSE_CARD_UPDATE_RFID_FAILED);
		STATEMACHINE_state = STATEMACHINE_STATE_IDLE;
		return;
	}
...
static void STATEMACHINE_dispense(void){
	if(!TCDMNG_payout(STATEMACHINE_usingTcdId)){
		STATEMACHINE_dispenseResultCallback(PROTOCOL_RESULT_CMD_DISPENSE_CARD_TCD_PAYOUT_FAILED);
		STATEMACHINE_state = STATEMACHINE_STATE_IDLE;
		return;
	}
...
//		For Testing
//		STATEMACHINE_lastTimeDispense = HAL_GetTick();
		STATEMACHINE_nbCardRequest--;
		if(STATEMACHINE_nbCardRequest == 0){
			STATEMACHINE_dispenseResultCallback(PROTOCOL_RESULT_SUCCESS);
			STATEMACHINE_state = STATEMACHINE_STATE_IDLE;
		}else {
...
//			STATEMACHINE_state = STATEMACHINE_STATE_WAIT_FOR_RFID_DETECTED;
	...
	if(STATEMACHINE_dispenseCorruptFlag){
		...
		STATEMACHINE_dispenseResultCallback(PROTOCOL_RESULT_CMD_DISPENSE_CARD_CORRUPT);
		STATEMACHINE_state = STATEMACHINE_STATE_IDLE;
		return;
	}
