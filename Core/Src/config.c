...
static bool CONFIG_set_default(CONFIG_t *_config, CONFIG_t *config_temp) {
  ...
  // Set password
  if (!CONFIG_field_is_empty((uint8_t *)config_temp->password,
                             sizeof(config_temp->password))) {
  ...
  // Set amount
  if (!CONFIG_field_is_empty((uint8_t *)(&config_temp->amount),
                             sizeof(config_temp->amount))) {
    memcpy(&_config->amount, &config_temp->amount, sizeof(_config->amount));
  }
  // Set card prices
  if (!CONFIG_field_is_empty((uint8_t *)(&config_temp->card_price),
                             sizeof(config_temp->card_price))) {
    memcpy(&_config->card_price, &config_temp->card_price,
           sizeof(_config->card_price));
  }
  // Set total_amount
  if (!CONFIG_field_is_empty((uint8_t *)(&config_temp->total_amount),
                             sizeof(config_temp->total_amount))) {
    memcpy(&_config->total_amount, &config_temp->total_amount,
           sizeof(_config->total_amount));
  }
  if (!CONFIG_field_is_empty((uint8_t *)(&config_temp->total_card),
                             sizeof(config_temp->total_card))) {
    memcpy(&_config->total_card, &config_temp->total_card,
           sizeof(_config->total_card));
  }
  if (!CONFIG_field_is_empty((uint8_t *)(&config_temp->total_card_by_day),
                             sizeof(config_temp->total_card_by_day))) {
    memcpy(&_config->total_card_by_day, &config_temp->total_card_by_day,
           sizeof(_config->total_card_by_day));
  }
  // Set card prices
  if (!CONFIG_field_is_empty((uint8_t *)(&config_temp->total_card_by_month),
                             sizeof(config_temp->total_card_by_month))) {
    memcpy(&_config->total_card_by_month, &config_temp->total_card_by_month,
           sizeof(_config->total_card_by_month));
  }
}
